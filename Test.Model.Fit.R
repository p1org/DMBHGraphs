...
########
Test.Model.Fit<-function(gdir, gbidir=graph.empty(vcount(gdir),directed=FALSE), zeros.dir=NULL, zeros.bidir=NULL, foldername, numSteps, iterations, mleMatr=NULL, model, ignore.trivial.moves=FALSE, tol=0.001, maxiter=100000, testname, plotlabel=NULL, ed.coin=c(1/3,1/3,1/3), nzconst.coin=c(ecount(gbidir)/(ecount(gdir)+ecount(gbidir)), ecount(gdir)/(ecount(gdir)+ecount(gbidir))), beta.SBM.coin=c(1/2), SBM.blocks=NULL,small.moves.coin=0, component.graphs = NULL){
  if (model == "beta.SBM"){
  ...
  for (i in 1:iterations){
    ...
    tmp = Estimate.p.Value.for.Testing(gdir, gbidir, model, zeros.dir, zeros.bidir, ignore.trivial.moves, mleMatr, steps.for.walk=numSteps, ed.coin,nzconst.coin, mle.maxiter, mle.tol, beta.SBM.coin, SBM.blocks, small.moves.coin,component.graphs)
...
########
Test.Model.Fit.Parallel<-function(gdir, gbidir=graph.empty(vcount(gdir),directed=FALSE), zeros.dir=NULL, zeros.bidir=NULL, foldername, numSteps, iterations, mleMatr=NULL, model, ignore.trivial.moves=FALSE, tol=0.001, maxiter=100000, testname, plotlabel=NULL, ed.coin=c(1/3,1/3,1/3), nzconst.coin=c(ecount(gbidir)/(ecount(gdir)+ecount(gbidir)), ecount(gdir)/(ecount(gdir)+ecount(gbidir))), beta.SBM.coin=c(1/2), SBM.blocks=NULL,small.moves.coin,component.graphs = NULL){
  if (model == "beta.SBM"){
    if (is.null(SBM.blocks) || !is.vector(SBM.blocks))       
      stop("beta.SBM model requires a non-empty vector SBM.blocks input." )     
    else if (length(SBM.blocks)!=vcount(gdir))       
